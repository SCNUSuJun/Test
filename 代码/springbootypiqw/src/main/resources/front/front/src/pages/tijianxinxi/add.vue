...
<div :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="体重" prop="tizhong">
            <el-input v-model="ruleForm.tizhong" 
                placeholder="体重" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="脉搏" prop="maibo">
            <el-input v-model="ruleForm.maibo" 
                placeholder="脉搏" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}'  label="皮肤病" prop="pifubing">
            <el-select v-model="ruleForm.pifubing" placeholder="请选择皮肤病"  >
              <el-option
                  v-for="(item,index) in pifubingOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}'  label="四肢缺失" prop="sizhiqueshi">
            <el-select v-model="ruleForm.sizhiqueshi" placeholder="请选择四肢缺失"  >
              <el-option
                  v-for="(item,index) in sizhiqueshiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="体检时间" prop="tijianshijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}'  label="体检结果" prop="tijianjieguo">
            <el-select v-model="ruleForm.tijianjieguo" placeholder="请选择体检结果"  >
              <el-option
                  v-for="(item,index) in tijianjieguoOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.tijianshijian = this.getCurDate()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='maibo'){
              this.ruleForm.maibo = obj[o];
              this.ro.maibo = true;
              continue;
            }
            if(o=='pifubing'){
              this.ruleForm.pifubing = obj[o];
              this.ro.pifubing = true;
              continue;
            }
            if(o=='sizhiqueshi'){
              this.ruleForm.sizhiqueshi = obj[o];
              this.ro.sizhiqueshi = true;
              continue;
            }
            if(o=='tijianshijian'){
        ...
        this.pifubingOptions = "无,有".split(',')
        this.sizhiqueshiOptions = "无,有".split(',')
        this.tijianjieguoOptions = "合格,不合格".split(',')
      },

    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('tijianxinxi/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('tijianxinxi/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('tijianxinxi/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
