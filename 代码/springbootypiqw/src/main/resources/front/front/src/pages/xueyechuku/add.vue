...
<div :style='{"border":"2px solid #a4d9b7","padding":"20px","margin":"20px auto 0","borderRadius":"8px","flexWrap":"wrap","background":"#fff","width":"1200px","position":"relative"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 0px","display":"inline-block"}' label="出库日期" prop="chukuriqi" >
              <el-date-picker
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('xueyechuku/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
        ...
        obj.daishu = obj.daishu - this.ruleForm.daishu
        if(obj.daishu<0){
        ...
        var obj = JSON.parse(localStorage.getItem('crossObj'));
        var table = localStorage.getItem('crossTable');
        obj.xueliang = obj.xueliang - this.ruleForm.xueliang
        if(obj.xueliang<0){
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post(table+`/update`,obj).then(res => {});
                          this.$http.post('xueyechuku/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
