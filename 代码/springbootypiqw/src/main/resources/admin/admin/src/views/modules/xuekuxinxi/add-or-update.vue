	...
	<div class="addEdit-block" :style='{"padding":"30px","backgroundAttachment":"fixed","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230312/52c1bcab43064c72aa492e0f342d2666.jpg)","flexDirection":"row-reverse","display":"flex","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","height":"calc(100vh - 120px)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.xuekubianhao" label="血库编号" prop="xuekubianhao">
					<el-input v-model="ruleForm.xuekubianhao" placeholder="血库编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="血液种类" prop="xueyezhonglei">
					<el-select :disabled="ro.xueyezhonglei" v-model="ruleForm.xueyezhonglei" placeholder="请选择血液种类" >
						<el-option
							v-for="(item,index) in xueyezhongleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="血液种类" prop="xueyezhonglei">
					<el-input v-model="ruleForm.xueyezhonglei"
						placeholder="血液种类" readonly></el-input>
				</el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
            ...
            this.xueyezhongleiOptions = "全血,血小板,红细胞,血浆".split(',')
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuekuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuekuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
